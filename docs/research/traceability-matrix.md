...
| RQ4 | Cơ chế Pool phù hợp VPS | Spike ba ứng viên, bảng điểm có điều kiện loại | Latency/RAM/connections; test bypass | 0 leak bắt buộc; điểm trọng số còn lại | ARC-04, SEC-10..12 | ADR lựa chọn Pool | `PENDING_DATA`; chưa quyết định |
| RQ4 | Storage phù hợp | Spike filesystem vs S3-compatible | Test namespace/quota/signed URL; footprint | 0 bypass; điểm bảo mật, vận hành, tài nguyên | FR-50..55, SEC-13 | ADR storage | `PENDING_DATA` |
| RQ4 | Payment sandbox phù hợp | Spike VNPay vs Stripe; adapter giả khi thiếu credential | Test chữ ký, callback trùng/sai thứ tự | Callback giả bị từ chối; idempotency đạt | FR-70..74, SEC-14 | ADR payment | `PENDING_DATA` |
