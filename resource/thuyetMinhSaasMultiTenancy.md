BỘ GIÁO DỤC VÀ ĐÀO TẠO

**ĐẠI HỌC CẦN THƠ**

# THUYẾT MINH ĐỀ TÀI

**NGHIÊN CỨU KHOA HỌC CỦA SINH VIÊN**

## 1. Tên đề tài

XÂY DỰNG ỨNG DỤNG QUẢN LÝ CÔNG VIỆC THEO KIẾN TRÚC ĐA THUÊ BAO

(Development of a Multi-Tenant Task Management Application)

## 2. Mã số đề tài

THS2026-

## Lĩnh vực ưu tiên (đánh dấu X)

- Khoa học cơ bản
- Công nghệ cao trong nông nghiệp, thủy sản và phát triển bền vững
- Môi trường, tài nguyên thiên nhiên và biến đổi khí hậu
- **Công nghệ, công nghệ thông tin và chuyển đổi số** (X)
- Khoa học giáo dục, luật và xã hội nhân văn
- Phát triển kinh tế, thị trường và nông thôn
- Công nghệ sinh học và thực phẩm
- Không thuộc 7 Lĩnh vực ưu tiên

## 3. Lĩnh vực nghiên cứu (đánh dấu X)

- Khoa học Tự nhiên
- **Khoa học Kỹ thuật và Công nghệ** (X)
- Khoa học Y, dược
- Khoa học Nông nghiệp
- Khoa học Xã hội
- Khoa học Nhân văn

## 4. Loại hình nghiên cứu (đánh dấu X)

- Cơ bản
- **Ứng dụng** (X)
- Triển khai

## 5. Thời gian thực hiện

06 tháng — Từ tháng 5 năm 2026 đến tháng 10 năm 2026

## 6. Đơn vị của chủ nhiệm đề tài

Tên đơn vị: Trường Công Nghệ Thông Tin & Truyền Thông (CNTT & TT)

Điện thoại: +84 0292 3831301

E-mail: webmaster@cit.ctu.edu.vn

Địa chỉ: Khu 2, đường 3/2, phường Xuân Khánh, quận Ninh Kiều, thành phố Cần Thơ

Họ và tên thủ trưởng đơn vị: Nguyễn Hữu Hòa

## 7. Chủ nhiệm đề tài

Họ và tên: Ngô Hưng Thịnh

Ngày tháng năm sinh: 21/04/2005

Điện thoại di động: 0939172285

E-mail: thinhb2303904@student.ctu.edu.vn

MSSV: B2303904

Lớp: Kỹ thuật phần mềm (CLC) (DI2396F2)

Khóa: 49

## 8. Những thành viên tham gia nghiên cứu đề tài

1. Ngô Hưng Thịnh — B2303904 (DI2396F2) — Khóa 49

   - Phân tích yêu cầu cho ứng dụng và thiết kế giải pháp kỹ thuật cho cơ chế đa thuê bao phù hợp
   - Thiết kế kiến trúc phần mềm cho ứng dụng quản lý công việc đa thuê bao
   - Xây dựng và triển khai ứng dụng dưới dạng dịch vụ phần mềm
2. Nguyễn Thanh Trọng — B2305615 (DI2396F1) — Khóa 49

   - Phân tích yêu cầu cho ứng dụng và thiết kế giải pháp kỹ thuật cho cơ chế đa thuê bao phù hợp
   - Thiết kế kiến trúc phần mềm cho ứng dụng quản lý công việc đa thuê bao
   - Xây dựng và triển khai ứng dụng dưới dạng dịch vụ phần mềm
3. Trang Minh Chánh — B2303864 (DI2396F1) — Khóa 49

   - Nghiên cứu tổng quan kiến trúc đa thuê bao và quản lý công việc theo mô hình Kanban
   - Phân tích yêu cầu cho ứng dụng và thiết kế giải pháp kỹ thuật cho cơ chế đa thuê bao phù hợp
   - Thực hiện kiểm thử và đánh giá
4. Huỳnh Hồng Ân — B2306657 (DI23V7F1) — Khóa 49

   - Nghiên cứu tổng quan kiến trúc đa thuê bao và quản lý công việc theo mô hình Kanban
   - Thực hiện kiểm thử và đánh giá
   - Viết báo cáo nghiên cứu
5. Cao Tường Hưng — B2303873 (DI2396F1) — Khóa 49

   - Phân tích yêu cầu cho ứng dụng và thiết kế giải pháp kỹ thuật cho cơ chế đa thuê bao phù hợp
   - Thiết kế kiến trúc phần mềm cho ứng dụng quản lý công việc đa thuê bao
   - Xây dựng và triển khai ứng dụng web dưới dạng dịch vụ phần mềm

### Cán bộ hướng dẫn sinh viên thực hiện đề tài

Họ và tên: TS. Ngô Bá Hùng, MSCB: 1124

Đơn vị công tác: Trường Công nghệ thông tin và truyền thông.

Lĩnh vực chuyên môn: Mạng máy tính, Quản trị mạng, Điện toán đám mây, Phần mềm tự do nguồn mở.

Nhiệm vụ: Hướng dẫn nội dung khoa học và hướng dẫn lập dự toán kinh phí đề tài

## 9. Đơn vị phối hợp chính

Không

## 10. Tổng quan tình hình nghiên cứu thuộc lĩnh vực của đề tài ở trong và ngoài nước

### 10.1. Trong nước

Tại Việt Nam, các hệ thống phần mềm cung cấp dưới dạng dịch vụ (Software as a Service - SaaS) theo mô hình đa thuê bao (Multi-tenant) đang ngày càng phổ biến trong tiến trình chuyển đổi số của doanh nghiệp, đặc biệt trong các lĩnh vực quản lý nội bộ, quản lý công việc và phối hợp nhóm, trong số. Hiện nay, các nghiên cứu trong nước chưa đi sâu về kiến trúc đa thuê bao mà tập trung chủ yếu vào hai hướng liên quan: triển khai ứng dụng sử dụng mô hình đa thuê bao cho nghiệp vụ cụ thể và tối ưu hóa hạ tầng, nền tảng đám mây hỗ trợ triển khai SaaS đa thuê bao.
Thứ nhất, nhóm nghiên cứu tập trung triển khai ứng dụng đa thuê bao theo hướng nghiệp vụ cụ thể. Nhiều khóa luận và công trình tại các trường đại học trong nước đã xây dựng thành công các hệ thống đa người dùng theo hướng đa thuê bao. Các công trình này mặc dù đảm bảo đặc điểm của ứng dụng đa thuê bao nhưng thường xây dựng tính năng đa thuê bao mang tính đặc thù cho bài toán cụ thể [1][2].
Thứ hai, nhóm các nghiên cứu chuyên sâu về tối ưu hóa hạ tầng và nền tảng điện toán đám mây hỗ trợ triển khai SaaS đa thuê bao. Song song với việc triển khai ứng dụng, các nhà nghiên cứu trong nước đã tập trung giải quyết các bài toán cốt lõi của vận hành đám mây như cân bằng tải, phân bổ tài nguyên và đảm bảo chất lượng dịch vụ (Quality of Service – QoS) [3][4].
Ngoài ra, các doanh nghiệp trong nước (VNG, Base.vn, KiotViet,...) cũng đã triển khai thành công SaaS dạng đa thuê bao trong sản phẩm của họ, tuy nhiên, các kỹ thuật của họ được giữ kín như bí mật thương mại và tài sản trí tuệ độc quyền.

### 10.2. Ngoài nước

Các công trình nghiên cứu ngoài nước tập trung đi sâu vào việc mô hình hóa các kiến trúc đa thuê bao chuẩn và giải quyết các bài toán tối ưu hóa tài nguyên ở tầng ứng dụng và tầng trung gian.
Ở góc độ học thuật, nghiên cứu của Olabanji, Fitch và Matthew [5] thông qua phương pháp lập bản đồ hệ thống đã khảo sát 64 công bố khoa học trong giai đoạn 2015–2022 liên quan đến kiến trúc SaaS đa thuê bao. Kết quả cho thấy phần lớn các nghiên cứu tập trung vào phân loại mô hình kiến trúc và phân tích các thách thức kỹ thuật, tuy nhiên đa số các giải pháp mới dừng lại ở mức khái niệm hoặc tối ưu hóa từng thành phần riêng lẻ. Narasayya và Chaudhuri [6] tiếp cận ở tầng dữ liệu, làm rõ các đánh đổi giữa hiệu năng, khả năng mở rộng và mức độ cô lập dữ liệu trong các kiến trúc đa thuê bao, trong khi Kumar [7] hệ thống hóa các nguyên tắc thiết kế và các vấn đề bảo mật cốt lõi của mô hình SaaS đa thuê bao.
Bên cạnh đó, công trình của Shinu Pushpan [8] đóng vai trò như một nghiên cứu hệ thống hóa tri thức, tổng hợp các thành phần kiến trúc, mẫu thiết kế và công nghệ phổ biến trong hệ thống đa thuê bao. Mặc dù cung cấp cái nhìn tổng quan, các thành phần này chủ yếu được trình bày dưới dạng rời rạc, chưa được liên kết thành một khung kiến trúc nhất quán gắn với một bài toán nghiệp vụ cụ thể.
Ở góc độ công nghiệp, các nền tảng lớn như Microsoft Azure [9], Amazon Web Services [10] và Google Cloud [11] đã công bố nhiều kiến trúc tham chiếu và tài liệu hướng dẫn xây dựng SaaS đa thuê bao. Tuy nhiên, các kiến trúc này mang tính triển khai cao và phụ thuộc chặt chẽ vào hệ sinh thái của nhà cung cấp, trong khi các kho mã nguồn mẫu như của AWS [12] hoặc các công cụ mã nguồn mở chủ yếu tập trung vào khía cạnh kỹ thuật đơn lẻ, thiếu một mô hình kiến trúc tổng thể ở mức trừu tượng.
Tổng quan cho thấy, kiến trúc đa thuê bao đã được nghiên cứu tương đối đầy đủ ở từng thành phần riêng lẻ. Nhưng vẫn còn thiếu các nghiên cứu xây dựng một kiến trúc phần mềm đa thuê bao nhất quán, được thiết kế và kiểm chứng trong bối cảnh một ứng dụng nghiệp vụ cụ thể có độ phức tạp đủ lớn. Qua đó, đề tài hướng tới xây dựng một ứng dụng quản lý công việc đa thuê bao, đảm bảo đầy đủ tính năng đa thuê bao và triển khai như dịch vụ trên Internet.

Tài liệu tham khảo

[1] “Tìm hiểu kiến trúc multi-tenant trong hệ thống phần mềm hỗ trợ chẩn đoán bệnh dựa trên ảnh y khoa”, 2023-2024. Báo cáo đồ án tốt nghiệp, Trường Đại học Bách Khoa - ĐHQG TP. Hồ Chí Minh.

[2] Đ. T. Quý, “Xây dựng website quản lý nội dung đa người dùng với kiến trúc multi-tenancy”, Khóa luận tốt nghiệp, Trường Đại học Kinh tế Quốc dân, Hà Nội, tháng 12, 2024.

[3] N. X. Phi, “Nâng cao hiệu năng cân bằng tải trên điện toán đám mây”, Luận án Tiến sĩ, Học viện Công nghệ Bưu chính Viễn thông, Hà Nội, 2022.

[4] N. K. Chiến, “Nghiên cứu giải pháp tự động điều chỉnh tài nguyên hiệu quả trong điện toán đám mây”, Luận án Tiến sĩ, Học viện Công nghệ Bưu chính Viễn thông, Hà Nội, 2019.

[5] D. Olabanji, T. Fitch, và O. Matthew, “Multi-tenancy in Cloud-native Architecture: A Systematic Mapping Study”, WSEAS Transactions on Computers, vol. 22, tr. 25-43, 2023. DOI: 10.37394/23205.2023.22.4.

[6] V. R. Narasayya and S. Chaudhuri, “Multi-Tenant Cloud Data Services: State-of-the-Art, Challenges and Opportunities,” Proceedings of the ACM SIGMOD International Conference on Management of Data (SIGMOD), 2022, pp. 1-26, DOI: 10.1145/3514221.3522566.

[7] R. Kumar, “Multi-Tenant SaaS Architectures: Design Principles and Security Considerations”, overview paper, 2025.

[8] S. Pushpan, “Multi-Tenant Architecture: A Comprehensive Framework for Building Scalable SaaS Applications,” International Journal of Scientific Research in Computer Science, Engineering and Information Technology, vol. 10, no. 6, pp. 860-867, Nov. 2024, DOI: 10.32628/CSEIT241061151.
[9] Microsoft, “Architect multitenant solutions on Azure”, Azure Architecture Center. [Online]. Available: https://learn.microsoft.com/en-us/azure/architecture/guide/multitenant/overview.  Accessed: Dec. 16, 2025.

[10] Amazon Web Services, “SaaS Architecture Fundamentals”, AWS Whitepaper. [Online]. Available: https://docs.aws.amazon.com/whitepapers/latest/saas-architecture-fundamentals. Accessed: Dec. 16, 2025.

[11] Google Cloud, “Best practices for enterprise multi-tenancy”, Google Cloud Architecture Center. [Online]. Available: https://cloud.google.com/kubernetes-engine/docs/concepts/multitenancy-overview Accessed: Dec. 16, 2025.

[12] Amazon Web Services, SaaS Reference Architecture on Amazon ECS, aws-samples, GitHub repository. [Online]. Available: https://github.com/aws-samples/saas-reference-architecture-ecs.  Accessed: Dec. 16, 2025.

### 10.3. Danh mục các công trình đã công bố thuộc lĩnh vực của đề tài của chủ nhiệm và những thành viên tham gia nghiên cứu

a) Của chủ nhiệm đề tài: Không

b) Của các thành viên tham gia nghiên cứu: Không

## 11. Tính cấp thiết của đề tài

Sự phát triển mạnh mẽ của điện toán đám mây đã thúc đẩy mô hình Phần mềm dưới dạng Dịch vụ (Software as a Service – SaaS) trở thành xu hướng phổ biến trong phát triển và cung cấp các ứng dụng hiện đại. Trong đó, kiến trúc đa thuê bao (Multi-tenancy) đóng vai trò then chốt, cho phép một hệ thống phần mềm duy nhất phục vụ đồng thời nhiều nhóm người dùng hoặc tổ chức khác nhau trên cùng một hạ tầng, góp phần tối ưu chi phí triển khai, vận hành và mở rộng hệ thống.
Trong môi trường đại học, nhu cầu sử dụng các ứng dụng quản lý công việc và dự án ngày càng tăng nhằm hỗ trợ giảng viên và sinh viên trong việc tổ chức, theo dõi và phối hợp thực hiện các nhiệm vụ học tập, nghiên cứu và làm việc nhóm. Các hệ thống quản lý công việc theo mô hình Kanban được sử dụng rộng rãi nhờ khả năng trực quan hóa trạng thái công việc và hỗ trợ cộng tác. Tuy nhiên, để một ứng dụng quản lý công việc có thể được triển khai dưới dạng dịch vụ dùng chung cho nhiều lớp học, nhóm nghiên cứu hoặc dự án khác nhau, việc thiết kế kiến trúc đa thuê bao là yêu cầu mang tính cấp thiết.
Trên thực tế, nhiều nền tảng quản lý công việc thương mại đã triển khai thành công mô hình đa thuê bao, nhưng các giải pháp này thường gắn chặt với hệ sinh thái của các nhà cung cấp đám mây hoặc được đóng gói như sản phẩm hoàn chỉnh, khó tiếp cận và phân tích ở góc độ kiến trúc. Trong khi đó, việc tự xây dựng một ứng dụng quản lý công việc đa thuê bao đòi hỏi phải giải quyết các bài toán kỹ thuật cốt lõi như định danh và quản lý không gian làm việc của từng thuê bao, cô lập dữ liệu giữa các thuê bao trên cùng hạ tầng, và đảm bảo tính nhất quán trạng thái công việc giữa nhiều người dùng trong cùng một không gian làm việc.
Về mặt học thuật, các nghiên cứu hiện nay chủ yếu tập trung vào từng cơ chế riêng lẻ của kiến trúc đa thuê bao hoặc các mô hình tổng quát ở mức trừu tượng cao, trong khi còn thiếu các nghiên cứu gắn kết các cơ chế này trong một ứng dụng nghiệp vụ cụ thể. Điều này cho thấy sự cần thiết của việc nghiên cứu và minh họa kiến trúc đa thuê bao thông qua một bài toán đủ phức tạp và có tính thực tiễn cao như ứng dụng quản lý công việc theo mô hình Kanban.
Xuất phát từ thực trạng đó, đề tài lựa chọn xây dựng ứng dụng quản lý công việc đa thuê bao làm đối tượng nghiên cứu và triển khai. Thông qua ứng dụng này, đề tài tập trung phân tích và hiện thực các cơ chế đa thuê bao cốt lõi ở tầng ứng dụng, đồng thời triển khai hệ thống dưới dạng dịch vụ phần mềm có thể truy cập qua Internet. Kết quả nghiên cứu vừa có ý nghĩa lý luận trong việc hệ thống hóa các cơ chế kiến trúc đa thuê bao cho một bài toán cụ thể, vừa có ý nghĩa thực tiễn khi cung cấp một mô hình tham khảo khả thi cho việc phát triển và triển khai các ứng dụng quản lý công việc đa thuê bao trong môi trường đại học.

## 12. Mục tiêu đề tài

### 12.1 Mục tiêu tổng quát

Nghiên cứu và xây dựng kiến trúc đa thuê bao cho ứng dụng quản lý công việc trong môi trường đại học, tập trung vào các cơ chế định danh, cô lập dữ liệu giữa các thuê bao, và đảm bảo nhất quán trạng thái công việc. Nghiên cứu đề xuất và thiết kế kiến trúc đa thuê bao cho ứng dụng này, thực hiện triển khai dưới dạng dịch vụ phần mềm có thể truy cập qua Internet. Từ đó, đề tài làm rõ cách thức thiết kế và triển khai một ứng dụng quản lý công việc đa thuê bao.

### 12.2 Mục tiêu cụ thể

- Nghiên cứu và phân tích các thành phần cốt lõi trong kiến trúc đa thuê bao cho ứng dụng quản lý công việc, bao gồm cơ chế định danh, cô lập dữ liệu từng thuê bao và cơ chế đảm bảo tính nhất quán trạng thái công việc.
- Xây dựng ứng dụng quản lý công việc đa thuê bao áp dụng kiến trúc đã đề xuất, triển khai dưới dạng dịch vụ cung cấp phần mềm trên Internet, phục vụ giảng viên và sinh viên.
- Thực nghiệm ứng dụng ở quy mô vừa phải nhằm đánh giá tính khả thi và hiệu quả của kiến trúc đa thuê bao được đề xuất.

## 13. Đối tượng, phạm vi nghiên cứu

### 13.1 Đối tượng nghiên cứu

Đối tượng nghiên cứu của đề tài tập trung vào hai thành phần chính: kiến trúc đa thuê bao và ứng dụng quản lý công việc, trong đó không gian làm việc (workspace) do một khách hàng tạo và quản lý được xác định là thuê bao. Mỗi  không gian làm việc có thể chứa nhiều dự án và được vận hành trên cùng một hạ tầng hệ thống. Cụ thể đối tượng nghiên cứu bao gồm:
Các cơ chế kỹ thuật cốt lõi trong kiến trúc đa thuê bao để đảm bảo các tính năng đa thuê bao:

- Cơ chế cô lập dữ liệu: Các mô hình và kỹ thuật để phân tách dữ liệu an toàn giữa các đơn vị thuê bao trên cùng một hệ thống cơ sở dữ liệu.
- Định danh và Điều hướng Tenant: Các giải pháp kỹ thuật để nhận diện chính xác nguồn gốc yêu cầu của người dùng và điều hướng vào đúng không gian làm việc.
- Bảo mật và Phân quyền: Hệ thống kiểm soát quyền truy cập đa cấp độ, đảm bảo tính riêng tư và ngăn chặn truy cập trái phép giữa các tenant.
- Tối ưu hóa tài nguyên: Các chiến lược quản lý kết nối và truy vấn để duy trì hiệu năng hệ thống khi quy mô mở rộng.
  Quy trình quản lý công việc theo phương pháp Kanban và nghiệp vụ quản lý không gian làm việc:
- Nghiên cứu các quy tắc nghiệp vụ như: người dùng được mời bởi thuê bao chỉ có thể truy cập board thuộc phạm vi của mình, thuê bao có thể xóa board nhưng người dùng không thể và cơ chế phân quyền cho người dùng cụ thể.
- Các thành phần chính của hệ thống: quản lý người dùng và phân quyền, quản lý dự án và công việc theo thứ bậc dựa, quản lý và chia sẻ tài nguyên phục vụ cho công việc
- Cơ chế theo dõi tiến độ và nhắc nhở thời hạn thực hiện công việc đa kênh.

### 13.2 Phạm vi nghiên cứu

Không gian nghiên cứu:

- Kiến trúc đa thuê bao: Tập trung nghiên cứu và thiết kế kiến trúc phần mềm hướng dịch vụ đa thuê bao. Trọng tâm là xây dựng các cơ chế nền tảng: Định danh thuê bao, Cô lập dữ liệu và Đồng bộ trạng thái cơ bản.
- Các nhóm dịch vụ chính mà ứng dụng cung cấp qua Internet bao gồm:
  - Dịch vụ quản lý không gian làm việc.
  - Dịch vụ quản lý dự án và công việc theo mô hình Kanban.
  - Dịch vụ thông báo công việc.
- Không gian triển khai ứng dụng bao gồm môi trường phát triển là Localhost và môi trường thử nghiệm thông qua việc triển khai ứng dụng lên một máy chủ ảo có cấu hình giới hạn để mô phỏng môi trường thực tế trên Internet.

Giới hạn nội dung nghiên cứu:

- Đề tài tập trung giải quyết các vấn đề cốt lõi về mặt kiến trúc hệ thống, không tập trung vào phát triển một sản phẩm thương mại hoàn chỉnh. Cụ thể:
- Về mặt chức năng:
  - Xây dựng module Quản trị hệ thống để khởi tạo thuê bao.
  - Xây dựng module Quản trị Tenant để quản lý công việc và thành viên.
  - Xây dựng module Thành viên để xem và tương tác cơ bản với công việc được giao.
- Về mặt kỹ thuật:
  - Tập trung nghiên cứu và triển khai mô hình phù hợp như Shared Database, Separate Schema và Shared Schema trên các hệ quản trị cơ sở dữ liệu.
  - Nghiên cứu cơ chế định tuyến dựa trên Subdomain hoặc Header để phân loại yêu cầu người dùng.
- Đề tài không tập trung vào các cơ chế đồng bộ thời gian thực phức tạp hay xử lý xung đột nâng cao giữa nhiều người dùng. Không đi sâu vào các bài toán tối ưu hạ tầng quy mô lớn, cân bằng tải phức tạp, tự động mở rộng, bảo mật hạ tầng mạng chuyên sâu. Không phát triển ứng dụng cho di động.

Thời gian nghiên cứu: 6 tháng (từ 3/2026 đến 8/2026)

Quy mô triển khai thực nghiệm:

- Nhóm nghiên cứu sẽ thiết lập từ 3 đến 5 thuê bao giả lập trên hệ thống.
- Mỗi thuê bao sẽ có khoảng 10 đến 20 tài khoản người dùng hoạt động đồng thời.

Lý giải sự chọn mẫu:

- Nhóm quyết định giới hạn phạm vi nghiệp vụ ở mô hình Kanban thay vì các công cụ lập lịch phức tạp để phù hợp với nguồn lực và quỹ thời gian 6 tháng. Chiến lược này ưu tiên chiều sâu kiến trúc hơn độ rộng tính năng, tập trung giải quyết triệt để bài toán cô lập dữ liệu trong môi trường đa thuê bao cũng như giải quyết các bài toán gặp phải trong quá trình phát triển ứng dụng. Việc lựa chọn môi trường đại học làm ngữ cảnh thực nghiệm cho phép nhóm tận dụng cơ cấu tổ chức sẵn có (bài tập lớn, niên luận, luận văn,...) để mô phỏng môi trường sản phẩm, tạo điều kiện thuận lợi nhất để kiểm chứng sản phẩm trên đối tượng người dùng thực tế.

## 14. Cách tiếp cận, phương pháp nghiên cứu

### 14.1 Cách tiếp cận

Nghiên cứu nghiệp vụ cốt lõi và giải quyết các bài toán kỹ thuật đa thuê bao. Đề xuất giải pháp từ cơ sở lý thuyết, phân tích và lựa chọn chiến lược cơ sở dữ liệu phù hợp nhất cho bài toán quản lý công việc, từ đó thiết kế kiến trúc hệ thống tổng thể.
Xây dựng ứng dụng quản lý công việc dựa trên kiến trúc đa thuê bao và triển khai như một dịch vụ phần mềm truy cập trên Internet, tìm ra hướng giải quyết các bài toán cụ thể khi phát triển ứng dụng.
Đánh giá bằng các phương pháp kiểm thử phần mềm để đo lường hiệu quả của kiến trúc trong việc đảm bảo tính cô lập dữ liệu và hiệu năng hệ thống.

### 14.2 Phương pháp nghiên cứu

Phương pháp nghiên cứu của đề tài được triển khai theo hướng kết hợp giữa phân tích, xây dựng ứng dụng cài đặt và thực nghiệm kiểm chứng.

Phân tích và tổng hợp lý thuyết:

- Nghiên cứu các nền tảng quản lý công việc hiện nay để xác định các yêu cầu nghiệp vụ cốt lõi.
- Nghiên cứu, phân tích và đề xuất giải pháp cho các bài toán kỹ thuật đa thuê bao như cô lập dữ liệu và các cơ chế, công nghệ hỗ trợ cho kiến trúc đa thuê bao phù hợp với ứng dụng.

Xây dựng kiến trúc hệ thống:

- Xây dựng mô hình kiến trúc phần mềm nhằm tích hợp cơ chế đa thuê bao vào trong ứng dụng quản lý công việc, đảm bảo kiến trúc có khả năng tách biệt logic xử lý dữ liệu của từng khách hàng.
- Xây dựng luồng xử lý yêu cầu của thuê bao xuyên suốt hệ thống, đảm bảo khả năng nhận diện định danh thuê bao trên mỗi tầng.
  Đặc tả cơ chế phân quyền truy cập cho nhiều loại thuê bao.

Xây dựng ứng dụng:

- Xây dựng quy trình cho phép một giáo viên đăng ký tài khoản, thanh toán và được hệ thống tự động cấp phát không gian lưu trữ, cơ sở dữ liệu và tên miền riêng. Tổ chức dự án theo cấu trúc phân cấp thứ bậc giúp quản lý chặt chẽ quy mô dự án lớn.
- Xây dựng cơ chế chia sẻ và tái sử dụng tài nguyên giữa các đầu việc khác nhau trong cùng dự án. Tích hợp hệ thống nhắc nhở thời hạn công việc qua đa kênh để tối ưu hiệu suất làm việc nhóm.

Thực nghiệm và kiểm định:

- Thực hiện và quan sát hiệu năng của hệ thống khi có một lượng người dùng ở quy mô vừa phải cùng hoạt động qua các kịch bản kiểm thử, đảm bảo việc truy vấn dữ liệu của tenant này không nhìn thấy dữ liệu tenant khác trên cùng cơ sở dữ liệu, hạ tầng.

Đánh giá:

- Dựa trên sản phẩm thực tế, đề tài đánh giá mức độ đáp ứng của các giải pháp đối với các bài toán kỹ thuật về: tốc độ triển khai cho khách hàng mới, độ tin cậy của cô lập dữ liệu, phân quyền, khả năng chịu tải và mở rộng khi số lượng người dùng tăng lên.

## 15. Nội dung nghiên cứu và tiến độ thực hiện

### 15.1 Nội dung nghiên cứu

- Nghiên cứu tổng quan kiến trúc đa thuê bao và quản lý công việc theo mô hình Kanban
- Phân tích yêu cầu cho ứng dụng và thiết kế giải pháp kỹ thuật cho cơ chế đa thuê bao phù hợp
- Thiết kế kiến trúc phần mềm cho ứng dụng quản lý công việc đa thuê bao
- Xây dựng và triển khai ứng dụng dưới dạng dịch vụ phần mềm
- Thực hiện kiểm thử và đánh giá
- Viết báo cáo nghiên cứu

### 15.2 Tiến độ thực hiện

| STT | Nội dung công việc                                                                   | Sản phẩm                                         | Thời gian (bắt đầu-kết thúc) | Người thực hiện (số tháng)                                             |
| --- | --------------------------------------------------------------------------------------- | -------------------------------------------------- | ---------------------------------- | ---------------------------------------------------------------------------- |
| 1   | Nghiên cứu tổng quan kiến trúc đa thuê bao và quản lý công việc theo Kanban | Báo cáo tổng quan lý thuyết                   | 3/2026 - 4/2026                    | Trang Minh Chánh (0,7); Nguyễn Thanh Trọng (0,7); Huỳnh Hồng Ân (0,6)  |
| 2   | Phân tích yêu cầu và thiết kế giải pháp kỹ thuật cho cơ chế đa thuê bao  | Tài liệu đề xuất giải pháp kỹ thuật       | 3/2026 - 4/2026                    | Trang Minh Chánh (0,7); Ngô Hưng Thịnh (0,7); Nguyễn Thanh Trọng (0,6) |
| 3   | Thiết kế kiến trúc phần mềm cho ứng dụng                                        | Tài liệu đặc tả kiến trúc                   | 4/2026 - 5/2026                    | Ngô Hưng Thịnh (0,7); Nguyễn Thanh Trọng (0,7); Cao Tường Hưng (0,6) |
| 4   | Xây dựng và triển khai ứng dụng                                                   | Ứng dụng web quản lý công việc đa thuê bao | 5/2026 - 7/2026                    | Nguyễn Thanh Trọng (1); Cao Tường Hưng (1); Ngô Hưng Thịnh (1)       |
| 5   | Thực hiện kiểm thử và đánh giá                                                  | Báo cáo kết quả kiểm thử                     | 7/2026                             | Huỳnh Hồng Ân (0,5); Cao Tường Hưng (0,5)                              |
| 6   | Viết báo cáo nghiên cứu                                                            | Báo cáo nghiên cứu khoa học                   | 8/2026                             | Trang Minh Chánh (0,5); Huỳnh Hồng Ân (0,5)                              |

## 16. Sản phẩm

- I. Xuất bản phẩm: Không
- II. Sản phẩm đào tạo (Luận văn tốt nghiệp đại học): Không
- III. Sản phẩm khoa học và công nghệ:
  - Ứng dụng web quản lý công việc đa thuê bao (01): Ứng dụng web quản lý công việc theo kiến trúc đa thuê bao, triển khai dưới dạng dịch vụ phần mềm cung cấp trên Internet. Dữ liệu được cô lập dữ liệu giữa các khách hàng và đáp ứng đầy đủ các nghiệp vụ quản lý công việc theo mô hình Kanban.
- IV. Sản phẩm theo quy định của Đại học Cần Thơ:
  - Bản tin (01) — Theo đúng quy định của Đại học Cần Thơ
  - Báo cáo tóm tắt (01) — Theo đúng quy định của Đại học Cần Thơ
  - Video clips (01) — Tối đa 02 phút, đầy đủ thông tin trọng tâm của đề tài

## 17. Phương thức chuyển giao kết quả nghiên cứu và địa chỉ ứng dụng

### 17.1 Phương thức chuyển giao

Toàn bộ kết quả nghiên cứu của đề tài sẽ được tổng hợp và bàn giao cho Trường Công nghệ Thông tin và Truyền thông, Đại học Cần Thơ. Các sản phẩm này được sử dụng làm tài liệu tham khảo và phát triển các hướng nghiên cứu tiếp theo liên quan đến kiến trúc phần mềm và hệ thống đa thuê bao.

### 17.2 Địa chỉ ứng dụng

Trường Công nghệ Thông tin và Truyền thông, Đại học Cần Thơ

## 18. Tác động và lợi ích mang lại của kết quả nghiên cứu

### 18.1 Đối với lĩnh vực giáo dục và đào tạo

Cung cấp một công cụ trực quan hỗ trợ các nhóm sinh viên và giảng viên quản lý công việc, tiến độ đồ án, bài tập lớn. Việc sử dụng ứng dụng giúp hiện đại hóa phương pháp làm việc nhóm trong môi trường giáo dục, rèn luyện tư duy quản lý dự án chuyên nghiệp cho người học, thay thế các phương thức quản lý thủ công rời rạc qua tin nhắn hay bảng tính truyền thống.

### 18.2 Đối với lĩnh vực khoa học và công nghệ

Đóng góp một nguồn tham khảo về cách thiết kế và xây dựng ứng dụng quản lý công việc dựa trên kiến trúc đa thuê bao, cho phép triển khai ứng dụng này dưới dạng dịch vụ. Đề tài làm rõ kiến trúc đa thuê bao và các cơ chế kỹ thuật liên quan. Qua đó, nghiên cứu cung cấp tài liệu tham khảo cho việc phát triển các hệ thống ứng dụng đa thuê bao có yêu cầu tương tự trong các lĩnh vực khác.

### 18.3 Đối với phát triển kinh tế - xã hội

Hỗ trợ quá trình chuyển đổi số cho các doanh nghiệp vừa và nhỏ hoặc các nhóm khởi nghiệp thông qua việc cung cấp giải pháp quản lý quy trình làm việc tinh gọn với chi phí thấp. Việc sử dụng mô hình SaaS với kiến trúc đa thuê bao giúp các doanh nghiệp này loại bỏ gánh nặng đầu tư hạ tầng máy chủ và đội ngũ vận hành riêng biệt, từ đó tối ưu hóa nguồn lực xã hội và thúc đẩy năng suất lao động tổng thể thông qua quy trình làm việc được số hóa.

### 18.4 Đối với tổ chức chủ trì và các cơ sở ứng dụng

Các sản phẩm của đề tài được bàn giao cho Trường Công nghệ Thông tin và Truyền thông, Đại học Cần Thơ để phục vụ công tác giảng dạy và làm nguồn tham khảo cho các đề tài nghiên cứu, đồ án và luận văn có liên quan trong tương lai.

## 19. Kinh phí thực hiện đề tài và nguồn kinh phí

**Kinh phí thực hiện:** 15.000.000 đồng

- Kinh phí ĐHCT cấp: 15.000.000 đồng
- Các nguồn khác: 0 đồng

| Stt | Khoản chi, nội dung chi                         |      Tổng kinh phí | Kinh phí ĐHCT cấp | Các nguồn khác |
| --- | ------------------------------------------------- | -------------------: | -------------------: | ----------------: |
| 1   | Chi tiền thù lao tham gia thực hiện đề tài |           12.125.000 |           12.125.000 |                 0 |
| 2   | Chi mua vật tư, nguyên, nhiên vật liệu      |                    0 |                    0 |                 0 |
| 3   | Chi văn phòng phẩm, in ấn                     |              150.000 |              150.000 |                 0 |
| 4   | Chi họp hội đồng đánh giá, nghiệm thu     |            2.725.000 |            2.725.000 |                 0 |
|     | **Tổng cộng**                             | **15.000.000** | **15.000.000** |       **0** |

---

TRƯỜNG CNTT-TT | CÁN BỘ HƯỚNG DẪN | CHỦ NHIỆM ĐỀ TÀI

TL. HIỆU TRƯỞNG

TRƯỞNG PHÒNG KHOA HỌC, CÔNG NGHỆ VÀ ĐỔI MỚI SÁNG TẠO
